...
function Product() {
  ...
  const img = product.map((values) => {
    const photo = values.picturecart;
    const mainphoto = photo.trim();
    return mainphoto;
  });
  ...
  return (
    <div>
      ...
      >
        <div style={{ padding: 20, flex: "30%" }}>
          <Image
            id="imgg"
            src={`https://shielded-wildwood-34672.herokuapp.com/images/${img}`}
            layout="responsive"
          />
        </div>
